...
3. `codex review --uncommitted` で codex のレビューを通し、**結果を必ずユーザーに伝える**
4. 指摘内容を取り込んで修正する（取り込まない判断をした場合は理由を添える）
...
コミット前のレビューは「実装完了時のワークフロー」に従う（`codex review --uncommitted`）。
ブランチ全体の差分を見直したいときは `codex review --base main` も利用できる。
