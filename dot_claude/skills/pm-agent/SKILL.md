...
model: sonnet
allowed-tools: AskUserQuestion, Bash(gh:*), Bash(git remote:*), Bash(mkdir:*), Bash(${CLAUDE_SKILL_DIR}/scripts/*:*), Bash(cat:*)
---
...
詳細・サンプルコマンドは `${CLAUDE_SKILL_DIR}/SETUP.md`、GraphQL は `${CLAUDE_SKILL_DIR}/GRAPHQL.md` を参照。パスは `${CLAUDE_SKILL_DIR}/scripts/` プレフィックスで呼び出す。
...
### フェーズ 3: 実行（3A / 3B / 3C はフェーズ 1 の選択によって分岐）

#### フェーズ 3A: 議事録 → タスク化（メインフロー）

##### ステップ 3A.1: 議事録のパース

1. アクションアイテムをキーワードパターンで抽出:
   - 動詞パターン: 「〜する」「〜したい」「〜が必要」
   - バグパターン: 「〜が遅い」「〜が動かない」
   - 日付パターン: 「〜月末」「〜日まで」
...
4. Ticket Type 分類はタイトルの絵文字プレフィックス（🏁 Epic, 🎯 Feature, 📋 Story, ⚙️ Task, 🐛 Bug）で判別し、`Ticket Type` フィールドにも同じ値を設定。`Priority` も Projects V2 Field で管理（ラベル不使用）。
...
${CLAUDE_SKILL_DIR}/scripts/pm-bulk-issues.sh /tmp/claude/issues.json \
...
${CLAUDE_SKILL_DIR}/scripts/pm-link-hierarchy.sh /tmp/claude/hierarchy.json \
  --repo "$REPO"
...
**参照**: チェックポイント機能（途中失敗時の再開）・Iteration 継承/分散配置は `${CLAUDE_SKILL_DIR}/SETUP.md`。
...
- 認証スコープ: `${CLAUDE_SKILL_DIR}/SETUP.md` の「前提条件」
- 各フィールド作成の GraphQL: `${CLAUDE_SKILL_DIR}/GRAPHQL.md`
...
**参照**: 修正に使うスクリプトは本ファイル冒頭の「スクリプト一覧」、詳細は `${CLAUDE_SKILL_DIR}/SETUP.md` の「スクリプト一覧」。

### フェーズ 4: Kanban Status 更新

```bash
# 単一 Issue
${CLAUDE_SKILL_DIR}/scripts/pm-project-fields.sh <issue> \
  --status "Done" --project N --owner login

# 子 Issue 一括
${CLAUDE_SKILL_DIR}/scripts/pm-bulk-status.sh \
  --parent <issue> --status "Done" --project N --owner login
```

Status を "Done" にした場合、Issue の Close も行うかユーザーに確認する。

**参照**: オプション詳細はスクリプト本体の `--help`、トラブルシューティングは `${CLAUDE_SKILL_DIR}/SETUP.md` の「トラブルシューティング」。
...
> - 複数 Issue 作成時は `pm-bulk-issues.sh` スクリプトを使用する
> - `Priority` は Projects V2 Field で管理（`pm-project-fields.sh --bulk` 使用）

> **禁止**:
>
> - ユーザー確認なしでの Issue 作成
> - 3 時間を超える Task の作成（分割を提案）
> - 複数 Issue をインライン（直接 `gh issue create` ループ）で作成
> - `priority:*` ラベルの作成（`Priority` フィールドで管理するため）
> - Issue State（Open / Closed）を Kanban Status（Todo / In Progress / Done）と混同すること
